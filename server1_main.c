...
int main (int argc, char *argv[])
{
	...
	if(argc != 2) {
		err_quit("\n# Wrong usage. Please run server is this way: './<prog_name> <port_number>' #\n");
	}
	...
	while(1) {
		...
		//while per ricezione di file multipli	
		while(1) {
			...
			if(nread == 0) {
				if(cntf == 0)
					err_quit("(%s) - Error: didnt read any char\n", prog_name);
				if(cntf == 1) 
			...
			cntf++; //incremento il counter di file ricevuti
			...
			FILE *filedescr = fopen(nomefile, "rb");
			struct stat st;
			int remain_data, tot;
			...
			if(nsent == 0) {
				err_quit("(%s) - Error: client is offline\n", prog_name);
			}
